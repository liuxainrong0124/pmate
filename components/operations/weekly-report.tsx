    ...
    sections: [
      {
        ...
        risks: ["次日留存下降0.8pp，需关注", "首屏加载时间增加0.3s，影响新用户体验"],
        metricChanges: [
      ...
      },
      {
        ...
        risks: ["REQ-004画像标签体系延期2天", "iOS适配缺乏iOS开发资源"],
        metricChanges: [],
      },
      {
      ...
      },
      {
        ...
        risks: ["推送频率投诉增加，需调整策略"],
        metricChanges: [],
      },
    ],
    nextWeekPlan: [
      "启动首屏加载性能优化专项（目标<1.5s）",
      "完成红色按钮方案全量推全",
      "618大促活动预热启动（6月10日）",
      "完成用户画像标签体系开发并提测",
      "调整推送策略：降低普通用户推送频率至周2次",
    ],
    overallMood: "neutral",
    keyTakeaways: [
...
export function WeeklyReport() {
  ...
  const handleCopy = async () => {
    ...
    text += `## 下周计划\n${report.nextWeekPlan.map((p, i) => `${i + 1}. ${p}`).join("\n")}\n`;
                    ...
                    {section.risks.length > 0 && (
                      <div>
                        ...
                        {section.risks.map((r, j) => (
                          <div key={j} className="flex items-start gap-1.5 text-xs text-amber-700 dark:text-amber-400">
