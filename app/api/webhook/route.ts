...
export async function POST(request: NextRequest) {
  try {
    ...
    const { feedbackText, source, metadata } = body;
    ...
    if (feedbackText.length > 100000) {
      return NextResponse.json(
        { error: "feedbackText must be under 100,000 characters" },
        { status: 400 }
      );
    }
    ...
    const response = await callLlm({
      messages: [
        ...
        { role: "user", content: buildFeedbackUserPrompt(feedbackText) },
      ],
      temperature: 0.3,
