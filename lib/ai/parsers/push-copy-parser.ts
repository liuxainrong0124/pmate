...
const VALID_STYLES = ["professional", "friendly", "urgent"];
...
export function parsePushCopyResponse(rawJson: string): GeneratedCopyVariant[] {
  ...
  return parsed.variants.map((v: any): GeneratedCopyVariant => ({
    style: VALID_STYLES.includes(v.style) ? v.style : "professional",
    title: String(v.title || ""),
    body: String(v.body || ""),
    cta: String(v.cta || ""),
    reasoning: String(v.reasoning || ""),
    estimatedOpenRate: String(v.estimatedOpenRate || ""),
    recommendedScenario: String(v.recommendedScenario || ""),
  }));
}
